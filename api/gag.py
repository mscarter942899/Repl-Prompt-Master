...
class GAGAdapter(GameAPIAdapter):
    def __init__(self):
        super().__init__('gag')
        self.values_url = 'https://bloxgrind.com/grow-a-garden/values'
        ...
        try:
            ...
            async with session.get(self.values_url, headers=headers) as response:
                if response.status == 200:
                    ...
                    blocks = re.split(r'<div[^>]*class="[^"]*(?:item-card|value-card|pet-card)[^"]*"', html)
                    ...
                    for block in blocks[1:]:
                        try:
                            ...
                            if icon_url and not icon_url.startswith('http'):
                                icon_url = f'https://bloxgrind.com{icon_url}'
                            ...
                            rarity = 'Common'
                            for r in ['Divine', 'Mythic', 'Legendary', 'Epic', 'Rare', 'Uncommon']:
                    ...
                    if items:
                        print(f"GAG: Fetched {len(items)} items from values site")
