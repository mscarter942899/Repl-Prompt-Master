from typing import List, Dict, Optional
import json
import os
from .base import GameAPIAdapter, APIRegistry

class GAGAdapter(GameAPIAdapter):
    def __init__(self):
        super().__init__('gag')
        self.base_url = 'https://growagarden.gg/api'
        self.fallback_path = 'data/fallback_gag.json'
        
    async def fetch_items(self) -> List[Dict]:
        cached = self._get_cached('items')
        if cached:
            return cached
        
        data = await self._request(f'{self.base_url}/items')
        if data and isinstance(data, list):
            items = [self.normalize_item(item) for item in data]
            self._set_cached('items', items)
            return items
        
        return self._load_fallback()
    
    async def fetch_item(self, item_id: str) -> Optional[Dict]:
        items = await self.fetch_items()
        for item in items:
            if item['id'] == item_id or item['normalized_name'] == item_id.lower():
                return item
        return None
    
    async def fetch_values(self) -> Dict[str, float]:
        cached = self._get_cached('values')
        if cached:
            return cached
            
        data = await self._request(f'{self.base_url}/values')
        if data and isinstance(data, dict):
            self._set_cached('values', data)
            return data
        
        items = await self.fetch_items()
        return {item['id']: item['value'] for item in items}
    
    async def fetch_icons(self) -> Dict[str, str]:
        items = await self.fetch_items()
        return {item['id']: item['icon_url'] for item in items if item.get('icon_url')}
    
    async def health_check(self) -> bool:
        try:
            data = await self._request(f'{self.base_url}/items')
            return data is not None
        except:
            return False
    
    def normalize_item(self, item: Dict) -> Dict:
        item_id = str(item.get('id', item.get('name', '')))
        name = item.get('name', 'Unknown')
        
        rarity_map = {
            'common': 'Common',
            'uncommon': 'Uncommon', 
            'rare': 'Rare',
            'epic': 'Epic',
            'legendary': 'Legendary',
            'mythic': 'Mythic',
            'divine': 'Divine'
        }
        rarity = rarity_map.get(item.get('rarity', '').lower(), 'Common')
        
        icon = item.get('icon', item.get('image', ''))
        if icon and not icon.startswith('http'):
            icon = f'https://growagarden.gg/images/{icon}'
        
        return {
            'id': item_id,
            'name': name,
            'normalized_name': self._normalize_name(name),
            'rarity': rarity,
            'icon_url': icon,
            'value': float(item.get('value', 0)),
            'tradeable': item.get('tradeable', True),
            'game': self.game_name,
            'metadata': {
                'category': item.get('category', ''),
                'type': item.get('type', '')
            }
        }
    
    def _load_fallback(self) -> List[Dict]:
        if os.path.exists(self.fallback_path):
            try:
                with open(self.fallback_path, 'r') as f:
                    data = json.load(f)
                    return [self.normalize_item(item) for item in data.get('items', [])]
            except:
                pass
        return []

def setup():
    APIRegistry.register('gag', GAGAdapter())
